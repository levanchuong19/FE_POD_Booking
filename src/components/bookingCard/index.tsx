...
const BookingCard: React.FC<ReservationCardProps> = ({ booking }) => {
  ...
  const onCheckboxChange: CheckboxProps['onChange'] = (e) => {
    if (e.target.checked) {
      setIsChoose(booking);
      console.log("Selected booking:", booking);  // Debugging log
    } else {
        ...
        const isMultipleDays = startDate.toDateString() !== endDate.toDateString();
            ...
            const calculateTotalServicePrice = () => {
              if (booking?.bookingServices) {
                  return booking.bookingServices.reduce((total, service) => total + service.totalPrice, 0);
              }
              return 0;
          };

            const adjustedTotalPrice = booking?.totalPrice ? booking.totalPrice - calculateTotalServicePrice() : 0;
    const calculateUsageHours = (startTime: Date, endTime: Date) => {
        const durationInMinutes = (endTime.getTime() - startTime.getTime()) / (1000 * 60);
        const hours = durationInMinutes / 60; 
        return hours > 0 ? hours : 1; 
    };
    const usageHours = booking?.startTime && booking?.endTime 
    ? calculateUsageHours(new Date(booking.startTime), new Date(booking.endTime)) : 1; 
  ...
  return (
    <div>
        <div className="reser" style={{ width:"100%"}}>
            ...
            <img style={{borderRadius:"10px"}} width={300} src={pods?.imageUrl} alt="" />
            </div>
            ...
            <p>Địa chỉ: {booking.locationAddress}</p>
             ...
             <p>{formatVND(pricePerHour)} x {booking?.startTime && booking?.endTime ? 
             calculateTime(new Date(booking.startTime), new Date(booking.endTime)) : ''}</p>
            {/* <p>{booking.bookingServices.map((service) => (<div key={service.serviceId}><p>{service.nameService}: {service.quantity}     Giá: {formatVND(service.totalPrice)}</p></div>))}</p>
            <p><strong>Giá: </strong>{formatVND(booking.totalPrice)}</p> */}  
            <Radio  style={{  marginRight: "20px" }}  onChange={onCheckboxChange}>Chọn</Radio>
